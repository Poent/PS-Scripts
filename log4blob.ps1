...
<#
    ...
    !Fork notes! !!!INCOMPLETE!!! (line 228 and convert write-host output to transcript or logfile. Also need to create folder with machineID )
    - Modifying script to upload results to write-only blob for log collection and centralized analysis of vulnerable systems. 
    write-only SAS key should be passed as -SAS <token> argument during script execution. 
    - Home Drive does not always equal root drive. Commonly configured for redirected user folders. Updating varScanScope option 1
    to target C:
#>
...
#upload log files to azure blob using sas key
###### INCOMPLETE, NEED TO INSERT AZCOPY COPY <LOCALPATH> <SASPATH> HERE TO UPLOAD RESULTS  ######
